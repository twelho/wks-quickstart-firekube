...
wksctl version
echo "Git url: $(git_http_url "$(git_remote_fetchurl "${git_remote}")")"
echo "Git branch: $(git_current_branch)"
wksctl init -v --git-url="$(git_http_url "$(git_remote_fetchurl "${git_remote}")")" --git-branch="$(git_current_branch)"
